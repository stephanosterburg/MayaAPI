...
MStatus	SineNode::initialize()
{
    ...
    // the input attibute is a time attribute that will be connected to the global time node attribute, time1.outTime
    //
...
MStatus SineNode::compute( const MPlug &_plug , MDataBlock &_data )
{
    ...
    if( _plug == m_output)
    {
        ...
        {
            case SINE :  out=amp*sin(freq*M_PI*Time.as(MTime::kSeconds));
                break;
            case COS :   out=amp*cos(freq*M_PI*Time.as(MTime::kSeconds));
                break;
            case COMPLEX:
                out=amp*cos(freq*M_PI*Time.as(MTime::kSeconds)) + amp*sin(2*freq*M_PI*Time.as(MTime::kSeconds));
                break;
        }
